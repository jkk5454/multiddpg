...
log_file = './data/train_multi_0112/reward.txt'
...
def learn_step(i,s,r, ep_reward,t1, env, action_base , frames, img_size=720,ddpg=None, max_episodes=MAX_EPISODES):
        ...
        for j in range(MAX_EP_STEPS):
            ...
            if(np.any(np.isnan(s_))):
                ...
                save_name = osp.join('./data/train_multi_0112/', 'ClothMove{}.gif'.format(i))
            ...
            if j == MAX_EP_STEPS-1:
                ...
                #top_view
                savename='./data/train_multi_0112/train{}_top.png'.format(i)
                show_depth(savename)
                ...
                #side view
                savename='./data/train_multi_0112/train{}_side.png'.format(i)
                show_depth(savename)
                center_x, center_y=pyflex.center_inf()
                with open(log_file, 'a') as f:
        ...
        if i and not i % TEST_PER_EPISODES:
            ...
            for j in range(MAX_EP_STEPS):
                ...
                if j == MAX_EP_STEPS - 1:
                    ...
                    #top vision
                    savename='./data/train_multi_0112/test/Test_{}_top.png'.format(i)
                    show_depth(savename)
                    center_x, center_y=pyflex.center_inf()
                    wrinkle_density, wrinkle_avedepth=pyflex.wrinkle_inf()
                    with open(log_file, 'a') as f:
                    ...
                    pyflex.set_camera_params(
                        np.array([*cam_pos2,*cam_angle2,720,720]))
                    #side vision
                    savename='./data/train_multi_0112/test/Test_{}_side.png'.format(i)
                    show_depth(savename)
                    center_x, center_y=pyflex.center_inf()
                    ...
                    '''
                    save_name = osp.join('./data/train_multi_0112/test/', 'ClothMove{}.gif'.format(i))
                    save_numpy_as_gif(np.array(frames), save_name)
                    print('Video generated and save to {}'.format(save_name))
                    '''
