...
std::tuple<py::array_t<unsigned char>, py::array_t<float>> pyflex_render(int capture, char *path)
{
    ...
    */
    ...
    for (int i = 0; i < g_screenWidth * g_screenHeight; ++i)
    {
        ...
        rendered_depth_ptr[i] = 2 * g_camFar * g_camNear / (g_camFar + g_camNear - (2 * rendered_depth_float_ptr[i] - 1.0) * (g_camFar - g_camNear));
        //std::cout<<rendered_depth_ptr[i]<<endl;
        //rendered_depth_ptr[i] =  4*(2 *g_camFar * g_camNear / (g_camFar + g_camNear - (2 * rendered_depth_float_ptr[i] - 1.0) * (g_camFar - g_camNear))-0.4);
        if(g_clothOnly){
            if(rendered_depth_ptr[i]<abs(cambase-workbenchbase)){      //0.7 based on the pose of camera
                int y = i / g_screenWidth;
                int x = i - y * g_screenWidth;
                center_y+=y;
                center_x+=x;
                clothvertices++;
...
/*wrinkle information*/
std::tuple<float,float> pyflex_wrinkle_inf() {
    return std::make_tuple(g_wrinkledensity, g_avedepth);
}

/*center information*/
std::tuple<float,float> pyflex_center_inf() {
...
PYBIND11_MODULE(pyflex, m)
{
    ...
    /*wrinkle infomation*/
    m.def("wrinkle_inf", &pyflex_wrinkle_inf);
